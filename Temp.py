...
```
Interval trigger: utterance_count > 0 and utterance_count % UTTERANCE_THRESHOLD == 0
...
if rules is None:
    # Fallback: matched categories OR every nth utterance
    if matched_categories:
        return True, f"fallback_matched_categories={matched_categories}"
    if utterance_count > 0 and utterance_count % UTTERANCE_THRESHOLD == 0:
        return True, f"fallback_utterance_interval={UTTERANCE_THRESHOLD} count={utterance_count}"
    return False, ""
...
utterances = []
matched_categories = []
logger.info(“Extracting utterances | segment_count=%s | segment_keys=%s”,
len(segments or []),
[list(s.keys()) for s in (segments or []) if isinstance(s, dict)])
...
logger.info("Extraction result | utterances_found=%s | categories=%s", len(utterances), matched_categories)
...
if event_type == "CONTACT_INITIATED":
    existing = redis_get(session_key)
    if existing:
        # Never overwrite existing session — CONTACT_INITIATED may arrive
        # out of order after SEGMENTS have already been stored (Kinesis
        # does not guarantee ordering across event types)
        logger.warning(
            "CONTACT_INITIATED received but session already exists — skipping overwrite | "
            "contact_id=%s | utterance_count=%s",
            contact_id,
            json.loads(existing).get("qualification_context", {}).get("utterance_count", 0),
        )
        return {"contact_id": contact_id, "event_type": event_type, "skipped": True}
...
invoke, reason = should_invoke_orchestrator(session, curr_count, matched_categories, speaker)
